/**
 * Try Chrome DevTools Protocol methods on the automation WebSocket
 */
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (this.callbacks.has(id)) {
          this.callbacks.delete(msg => true);
...
async function main() {
  ...
  try {
    ...
    await sleep(500);
    console.log('Initial messages:', client.allMessages.length);
    if (client.allMessages.length > 0) {
      console.log('First msg:', JSON.stringify(client.allMessages[0]).substring(0, 200));
    }

    // Try various CDP commands
    const cmds = [
    ...
    for (const [method, params] of cmds) {
      try {
        ...
        if (method === 'Page.captureScreenshot' && result && result.data) {
          ...
          const outPath = path.join(EVIDENCE_DIR, 'cdp-screenshot.png');
          fs.writeFileSync(outPath, imgBuf);
          console.log('  SAVED screenshot:', imgBuf.length, 'bytes');
        }
      } catch(e) {
