"""
qa_388_result_ui.py — STORY-00388 result UI upgrade QA
Captures fail result screen (star-dust) and victory result screen (sparkle).
Prereq: diffMap = [5, 5, 5, 5, 5]
"""
import sys, os, time
...
def find_devtools_hwnd():
    ...
    candidates = []
    def callback(hwnd, _):
        procId = ctypes.wintypes.DWORD()
        user32.GetWindowThreadProcessId(hwnd, ctypes.byref(procId))
        try:
            out = subprocess.check_output(
                f'tasklist /FI "PID eq {procId.value}" /NH /FO CSV',
...
def main():
    print("qa_388_result_ui.py — STORY-00388 result UI QA")
    ...
    # Step 3: Start Level 1 (will fail in 5s)
    print("\n[3] Click Level 1...")
    game_click(300, 160, "focus")
    time.sleep(0.4)
    ...
    time.sleep(2)
    capture(hwnd, "STORY-00388-01-game.png", "game screen")

    # Step 4: Wait for fail (timer=5s + 2s load = 7s)
    print("\n[4] Waiting 10s for fail result...")
    time.sleep(10)
    capture(hwnd, "STORY-00388-02-fail.png", "fail result — star-dust particles")

    # Step 5: Take 2nd fail screenshot 2s later (particles animated)
    print("\n[5] Second fail screenshot (particles animation check)...")
    time.sleep(2)
    capture(hwnd, "STORY-00388-03-fail-2s.png", "fail result 2s later")

    print("\n✓ Fail screenshots saved.")
    print("Manual check: look for animated star-dust dots in fail screenshots.")
    print("\nFor victory screenshots, complete a level manually and")
    print("run: python scripts/qa_388_result_ui.py --victory-only")

    print("\nEvidence files:")
    print("  STORY-00388-01-game.png")
    print("  STORY-00388-02-fail.png")
    print("  STORY-00388-03-fail-2s.png")
